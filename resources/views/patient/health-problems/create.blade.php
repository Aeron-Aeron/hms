    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('patient.health-problems.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" rows="5" required class="w-full px-3 py-2 border rounded-lg">{{ old('description') }}</textarea>
                            @error('description') <div class="text-red-500 text-sm">{{ $message }}</div> @enderror
                        </div>

                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Symptoms (comma separated)</label>
                            <input type="text" name="symptoms" value="{{ old('symptoms') }}" placeholder="e.g. fever,cough,headache"
                                   class="w-full px-3 py-2 border rounded-lg">
